...
def _ensure_last_login_date(df: pd.DataFrame, run_date: pd.Timestamp) -> pd.DataFrame:
    """
    If 'last_login_date' exists: coerce to YYYY-MM-DD.
    Else if 'last_login_days_ago' exists: derive as run_date - days.
    Else: leave blank (row will be excluded by windowing); warn once.
    """
    ...
    # no usable source -> blank; windowing will drop
    if not getattr(_ensure_last_login_date, "_warned", False):
        log.warning("[mobile] last_login_date not found and cannot be derived; affected rows will be skipped by windowing")
...
def fetch_candidates(run_date) -> pd.DataFrame:
    ...
    """
    path = settings.MOCK_DATA_CONFIG.get("mobile_data_file", "data/mock/mobile_data.csv")
    if not os.path.exists(path):
        log.warning("[mobile] mock file not found: %s", path)
        return _empty_df()
